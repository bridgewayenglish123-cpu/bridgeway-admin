...
export default function UploadReportModal({
  ...
  const checkWord = async (w: string) => {
    if (!w.trim()) return;
    try {
      const res = await fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${encodeURIComponent(w.trim())}`);
      if (!res.ok) setWordWarning(`"${w}" may not be a valid English word. Add anyway?`);
      else setWordWarning("");
    } catch { setWordWarning(""); }
  };

  const checkPhrase = async (p: string) => {
    if (!p.trim()) return;
    ...
    try {
      ...
      if (!res.ok) setPhraseWarning(`"${p}" may contain an error. Add anyway?`);
      else setPhraseWarning("");
    } catch { setPhraseWarning(""); }
  };
