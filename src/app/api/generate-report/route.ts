...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: student } = await admin
      .from("students")
      .select("id, en_name, zh_name, zoom_email, learning_goal, level, learner_type")
...

{
  ...
  "parent_summary": "[CRITICAL] If learner_type is Young Learner: MUST return a non-null string in Traditional Chinese (2-3 sentences for parents). Cover: what the child learned today, one proud moment of progress, one thing to encourage at home. NEVER return null for Young Learner. Example: \"今天 Mia 學習了日常生活詞彙，能夠用完整句子回答問題，進步非常明顯！建議在家可以和她練習說說看今天做了什麼。\" If learner_type is NOT Young Learner: return null.",
  "analysis_zh": {
